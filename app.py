...
def main():
    ...
    st.set_page_config(page_title="ProSpectAI: Message Generator", layout="wide")

    # Title of the app
    st.title("ProSpectAI: Message Generator")
    st.markdown("Generate personal resume-tailored messages based on the job posting.")
    ...
    if st.button("Generate Message"):
        if job_url:
            ...
            thought, response = generate_message_for_job(job_url)
...
# Dummy function to simulate message generation
def generate_message_for_job(job_url):
    # Load the resume
    loader = ResumeLoader()
